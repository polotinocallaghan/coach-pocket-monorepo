    ...
    const handlePlayerSelect = (sourceId: string) => {
        ...
        if (selectedType === 'session') {
            ...
            onSave({
                ...
                type: selectedType as 'match' | 'event',
                sourceId: sourceId
            });
        }
    };
                        ...
                        <>
                            ...
                            {sources.length === 0 ? (
                                ...
                                <div className="grid grid-cols-2 gap-6 h-[400px]">
                                    {/* Groups Column */}
                                    <div className="flex flex-col h-full overflow-hidden border border-slate-700/50 rounded-xl bg-slate-900/30 p-3">
                                        <div className="flex items-center gap-2 px-1 mb-3 text-white font-semibold text-sm">
                                            <Users className="w-4 h-4 text-blue-400" />
                                            Groups
                                        </div>
                                        <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                                            {sources.filter(s => s.type === 'team' && s.id !== 'user').map((source) => (
                                                <button
                                                    key={source.id}
                                                    onClick={() => handlePlayerSelect(source.id)}
                                                    className="w-full flex items-center gap-3 p-3 rounded-xl border border-slate-700 bg-slate-700/30 hover:bg-slate-700 transition-all text-left group"
                                                >
                                            ...
                                            {sources.filter(s => s.type === 'team' && s.id !== 'user').length === 0 && (
                                                <div className="text-center py-8 text-slate-600 text-xs italic">No groups found</div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col h-full overflow-hidden border border-slate-700/50 rounded-xl bg-slate-900/30 p-3">
                                        ...
                                        <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                                            {sources.filter(s => s.type === 'person' && s.id !== 'user').map((source) => (
                                                <button
                                                    key={source.id}
                                                    onClick={() => handlePlayerSelect(source.id)}
                                                    className="w-full flex items-center gap-3 p-3 rounded-xl border border-slate-700 bg-slate-700/30 hover:bg-slate-700 transition-all text-left group"
                                            ...
                                            {sources.filter(s => s.type === 'person' && s.id !== 'user').length === 0 && (
                                                <div className="text-center py-8 text-slate-600 text-xs italic">No players found</div>
                                            )}
                                        </div>
                                    </div>
                                </div>
