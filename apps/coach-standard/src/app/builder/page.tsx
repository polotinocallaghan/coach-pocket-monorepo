...
import { Search, ArrowLeft, Check, Save, Clock, Filter, ChevronDown, Star, X, BookMarked, ChevronLeft, ChevronRight, Target, Users, School, Repeat, Lightbulb, Zap, Printer, Plus, Minus, Flame, Activity, Brain } from 'lucide-react';
...
function SessionBuilderContent() {
    ...
    // Derive current player from sessionSourceId (must be after sessionSourceId state)
    const currentPlayer = useMemo(() => {
        if (!sessionSourceId || sessionSourceId === 'user') return null;
        return calendarSources.find(s => s.id === sessionSourceId && s.type === 'person') || null;
    }, [sessionSourceId, calendarSources]);
                        ...
                        <div className="max-w-7xl mx-auto px-6 py-4">
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    {/* Player Insights toggle — shows whenever builder has a player */}
                                    {currentPlayer && (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                showInsightsPanel
                                                    ...
                                                    : 'bg-slate-700/60 border-slate-600 text-slate-300 hover:border-indigo-500 hover:text-indigo-300'
                                            )}
                                            title="Toggle player training preferences"
                                        >
                    ...
                    <AnimatePresence>
                        {showInsightsPanel && currentPlayer && (() => {
                            ...
                            const PREF_FEATURES = [
                                ...
                                { key: 'technique', label: 'Technique',         color: 'from-blue-600/60 to-blue-400' },
                                { key: 'tactics',   label: 'Tactics',           color: 'from-purple-600/60 to-purple-400' },
                            ...
                            return (
                                <motion.div
                                    ...
                                    className="sticky top-[73px] z-20 max-w-7xl mx-auto px-6 pt-4"
                                >
                                    <div className="bg-slate-900/95 backdrop-blur-xl border border-indigo-500/30 rounded-2xl p-5 shadow-2xl shadow-indigo-500/10">
                                        {/* Header */}
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="flex items-center gap-3">
                                                <div className="p-1.5 bg-indigo-500/15 border border-indigo-500/30 rounded-lg">
                                                    <Activity className="w-4 h-4 text-indigo-400" />
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-white">{currentPlayer.name}'s Training Preferences</p>
                                                    <p className="text-[10px] text-slate-500">What this player values — design around it</p>
                                                </div>
                                                {priorities.length > 0 && (
                                                    <div className="flex items-center gap-1.5 ml-2">
                                                        <Flame className="w-3.5 h-3.5 text-orange-400" />
                                                        <span className="text-[11px] font-semibold text-orange-400">{priorities.map(p => p.label).join(' · ')}</span>
                                                    </div>
                                                )}
                                            </div>
                                            <button
                                        ...
                                        {/* Content: preferences or empty state */}
                                        {hasPrefs ? (
                                                    ...
                                                    return (
                                                        <div key={f.key} className="flex items-center gap-3 group">
                                                            <div className="w-28 flex items-center gap-1.5 flex-shrink-0">
                                                                {isPriority && (
                                                                    <Flame className="w-3 h-3 text-orange-400 fill-orange-400/30 flex-shrink-0" />
                                                                )}
                                                                <span className={cn(
                                                                    'text-[11px] font-semibold truncate',
                                                                    isPriority ? 'text-orange-300' : 'text-slate-300'
                                                                )}>{f.label}</span>
                                                            </div>
                                                            <div className="flex-1 h-2 bg-slate-800 rounded-full overflow-hidden border border-slate-700">
                                                                <motion.div
                                                                    ...
                                                                    className={cn('h-full rounded-full bg-gradient-to-r', f.color, isPriority && 'shadow-[0_0_6px_rgba(249,115,22,0.5)]')}
                                                                />
                                                            </div>
                                                            <span className="text-[10px] font-bold text-slate-500 w-7 text-right flex-shrink-0">
                                                                {isPriority ? '🔥' : `${pref?.value ?? 0}/5`}
                                                            </span>
                                                        </div>
                                                    );
                                                })}
                                            ...
                                            <div className="flex items-center gap-3 py-2 px-3 bg-slate-800/60 rounded-xl border border-slate-700/50">
                                                <Brain className="w-4 h-4 text-slate-500 flex-shrink-0" />
                                                <p className="text-xs text-slate-400">
                                                    <span className="font-semibold text-slate-300">{currentPlayer.name}</span> hasn't set training preferences yet.
                                                    They can configure them inside their Player Portal.
                                                </p>
                                            </div>
                                        )}
                                    </div>
